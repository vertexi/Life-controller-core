...
std::string get_last_append_line() { return last_append_line; }
}  // namespace life_controller_core
...
int create_event_log(std::string event_name, std::string event_description)
{
    ...
    std::string log_data =
        "CREATE  "
        ", " +
        get_time_str(&current) + ", " + "00:00:00, " + event_name + ", " + event_description + ", " +
        std::to_string(current) + ", " + std::to_string(current) + "\n";
    append_log(LOGFILE_NAME, log_data);
    return 0;
}
                    ...
                    std::time_t goal_end_time_t)
{
    ...
    std::string goal_description =
        timesOrduration ? "times: " + std::to_string(quantity) : "duration: " + std::to_string(quantity);

    std::string log_data =
        "GOAL    "
        ", " +
        goal_start_time_str + ", " + goal_duration_str + ", " + event_name + ", " + goal_description + ", " +
        std::to_string(goal_start_time_t) + ", " + std::to_string(goal_end_time_t) + "\n";
    append_log(LOGFILE_NAME, log_data);
    return 0;
}
